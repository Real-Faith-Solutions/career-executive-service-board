@extends('layouts.app')
@section('title', 'Classification Basis Title - Edit')
@section('content')
...
<div class="relative my-10 overflow-x-auto shadow-lg sm:rounded-lg">
    <div class="w-full text-left text-gray-500">
        <div class="bg-blue-500 uppercase text-gray-700 text-white">
            <h1 class="px-6 py-3">
                Classification Basis Title
            </h1>
        </div>

        <div class="bg-white px-6 py-3">
            <form action="{{ route('library-class-basis.update', $datas->cbasis_code) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="sm:gid-cols-1 mb-3 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <div class="mb-3">
                        <label for="basis">Basis<sup>*</span></label>
                        <input id="basis" name="basis" type="text" value="{{ $datas->basis }}" required>
                        @error('basis')
                    ...
                    <div class="mb-3">
                        ...
                        <input id="classdate" name="classdate" type="date" value="{{ $datas->classdate }}" required>
                        @error('classdate')
                        <span class="invalid" role="alert">
                            <p>{{ $message }}</p>
                        </span>
