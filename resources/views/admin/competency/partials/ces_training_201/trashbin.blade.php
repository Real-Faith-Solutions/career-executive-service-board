    ...
    <table class="w-full text-left text-sm text-gray-500">
        ...
        <tbody>
            ...
            <tr class="border-b bg-white">
                ...
                <td class="px-6 py-3">
                    {{ $competencyCesTrainingTrashedRecords->cesTrainingPersonalData->title ?? '' }}
                ...
                <td class="px-6 py-4 text-right uppercase">
                    <div class="flex">
                        <form
                            action="{{ route('ces-training.restore', ['ctrlno'=>$competencyCesTrainingTrashedRecords->pid]) }}"
                            method="POST"
                            id="restore_training_participant_form{{$competencyCesTrainingTrashedRecords->pid}}">
                            @csrf
                            <button type="button"
                                id="restoreTrainingParticipantButton{{$competencyCesTrainingTrashedRecords->pid}}"
                        ...
                        <form
                            action="{{ route('ces-training.forceDelete', ['ctrlno'=>$competencyCesTrainingTrashedRecords->pid]) }}"
                            method="POST"
                            id="permament_training_participant_form{{$competencyCesTrainingTrashedRecords->pid}}">
                            @csrf
                            @method('DELETE')
                            <button type="button"
                                id="permanentDeleteTrainingParticipantButton{{$competencyCesTrainingTrashedRecords->pid}}"
                                onclick="openConfirmationDialog(this, 'Confirm Permanent Deletion', 'Are you sure you want to permanently delete this info?')">
