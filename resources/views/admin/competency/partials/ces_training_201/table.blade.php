    ...
    <table class="w-full text-left text-sm text-gray-500">
        ...
        <tbody>
            ...
            <tr class="border-b bg-white">
                ...
                <td class="px-6 py-3">
                    {{ $training->cesTrainingPersonalData->title ?? '' }},
                    {{ $training->cesTrainingPersonalData->lastname ?? '' }},
                    {{ $training->cesTrainingPersonalData->firstname ?? '' }}
                    {{ $training->cesTrainingPersonalData->name_extension ?? '' }}
                    {{ $training->cesTrainingPersonalData->middleinitial ?? '' }}
                </td>
                ...
                <td class="px-6 py-4 text-right uppercase">
                    <div class="flex">
                        <form
                            action="{{ route('ces-training.edit', ['ctrlno'=>$training->pid, 'cesno'=>$training->cesTrainingPersonalData->cesno]) }}"
                            method="GET">
                            @csrf
                        ...
                        <form action="{{ route('ces-training.destroy', ['ctrlno'=>$training->pid]) }}" method="POST"
                            id="delete_training_participant_form{{$training->pid}}">
                            @csrf
                            @method('DELETE')
                            <button type="button" id="deleteTrainingParticipantButton{{$training->pid}}"
