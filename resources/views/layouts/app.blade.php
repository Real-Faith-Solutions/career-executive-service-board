...
<body>
    ...
    <div id="profile-avatar-modal"
        ...
        <div class="modal-content bg-white p-6 rounded-lg shadow-lg">
            <form id="uploadFormAvatar" action="{{ route('/upload-avatar-profile-201', ['cesno'=>$cesno]) }}"
                ...
                <input type="file" id="imageInputAvatar" name="imageInput"
                ...
                <button type="submit" name="submit" id="uploadButtonAvatar"
                    class="px-6 py-3 bg-blue-500 text-white rounded-md shadow hover:bg-blue-600 transition-colors duration-300">Upload</button>
            </form>
        </div>
    </div>
    {{-- end --}}
    ...
    <div id="confirmationBackdrop"
        ...
        <div id="confirmationDialog" class="bg-white w-1/3 p-6 rounded-lg shadow-lg hidden" data-form-id="">
            ...
            <div class="text-right">
                <button class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg mr-2 hover:bg-gray-400"
                    onclick="closeConfirmationDialog()">No</button>
    ...
    <div id="twoFactorConfirmationBackdrop"
        class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 hidden">
        <div id="twoFactorConfirmationDialog" class="bg-white w-1/3 p-6 rounded-lg shadow-lg hidden">
            <h2 class="text-lg font-bold mb-4" id="twoFactorConfirmationDialogTitle"></h2>
            <p class="mb-4" id="twoFactorConfirmationDialogStatement"></p>
            <div class="text-right">
                <button class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg mr-2 hover:bg-gray-400"
                    onclick="closetwoFactorConfirmationDialog()">No</button>
                <button class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                    onclick="confirmTwoFactorConfirmationDialog()">Yes</button>
            </div>
        </div>
    </div>
    {{-- end --}}
    ...
    <div id="add-medical-history-modal"
        class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 hidden">
        <div class="modal-content bg-white p-6 rounded-lg shadow-lg">
            <form id="addMedicalHistoryForm" action="{{ route('medical-history.store', ['cesno'=>$cesno]) }}"
                method="POST" class="flex flex-col items-center"
                onsubmit="return checkErrorsBeforeSubmit(addMedicalHistoryForm)">
                @csrf
                ...
                <button type="submit" id="addMedicalHistoryBtn"
    ...
    <div id="resend_email_modal"
        class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 hidden">
        <div class="modal-content bg-white p-6 rounded-lg shadow-lg">
            <form id="resendEmailForm" action="{{ route('resend-email', ['cesno' => $cesno]) }}" method="POST"
                ...
                <button type="submit" id="resendEmailBtn"
                    class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Resend</button>
            </form>
        </div>
    </div>
    {{-- end --}}
