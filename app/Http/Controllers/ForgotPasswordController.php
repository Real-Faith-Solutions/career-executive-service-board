...
class ForgotPasswordController extends Controller
{
    ...
    public function getPasswordResetPage(Request $request, $token){
        ...
        if(!Auth::check()){
            ...
            return view('reset_password', ['token' => $token, 'message_status' => '', 'email' => $request->email]);
        ...
        if(!Auth::check()){
            ...
            $validator = Validator::make(
                ...
                array(
                    'token' => 'required',
                    'email' => 'required|email',
                    'password' => ['required','regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W])[A-Za-z\d\W]{8,}$/','confirmed'],
                    'password_confirmation' => ['required','regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W])[A-Za-z\d\W]{8,}$/'],
                )
