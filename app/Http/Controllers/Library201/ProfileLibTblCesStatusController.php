...
class ProfileLibTblCesStatusController extends Controller
{
    ...
    {
        $codeExist = ProfileTblCesStatus::withTrashed()->where('cesstat_code', $code)->exists();
        
        if($codeExist)
        {
            return redirect()->back()->with('error', 'The CES Status already has user, so it cannot be deleted !!');
        }
        else
        {
            $profileLibTblCesStatus = ProfileLibTblCesStatus::find($code);        
